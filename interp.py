"""EpicLang Interpreter - A simple interpreter for the EpicLang language."""
...
def main() -> None:
    ...
    class ErrorHandler(ErrorListener):
        """Handler for syntax errors during lexing and parsing."""

        def syntaxError(self, *args: Any) -> None:
            print(SYNTAX_ERROR_MSG)
            sys.exit(1)

    class FunctionTableBuilder(EpicLangVisitor):
    ...
    class InterpreterVisitor(EpicLangVisitor):
        """Executes the program by visiting the AST."""

        # Binary operators - defined as class constant to avoid recreation on each call
        BINARY_OPS = {
            '+': lambda a, b: a + b,
            '-': lambda a, b: a - b,
            '*': lambda a, b: a * b,
        ...
        # Unary operators - defined as class constant to avoid recreation on each call
        UNARY_OPS = {
            '+': lambda x: x,
            '-': lambda x: -x,
            '!': lambda x: not x,
        ...
        def visitBinExpr(self, ctx: Any) -> Any:
            ...
            if type(left) != type(right):
                self.runtime_error()
            try:
                return self.BINARY_OPS[ctx.op.text](left, right)
        ...
        def visitUnaryExpr(self, ctx: Any) -> Any:
            ...
            try:
                return self.UNARY_OPS[ctx.op.text](self.visit(ctx.expr()))
            except (TypeError, ValueError, ZeroDivisionError):
                self.runtime_error()
